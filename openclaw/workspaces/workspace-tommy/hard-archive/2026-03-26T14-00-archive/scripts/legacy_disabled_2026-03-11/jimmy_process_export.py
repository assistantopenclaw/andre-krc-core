...
from run_helpers import FINISHED_DIR, FINISHED_DRIVE_FOLDER_ID, infer_drive_doc_name, append_log, load_manifest, write_manifest
...
def main():
    ...
    # Optional: upload to Google Drive as Google Docs and store link in manifest
    try:
        doc_name = infer_drive_doc_name(run_id, finished_path)
        cmd = [
            'gog','drive','upload', finished_path,
        ...
        j = json.loads(out)
        fmeta = j.get('file') or j
        m.setdefault('drive', {})
        m['drive'].update({
            'file_id': fmeta.get('id'),
            'folder_id': FINISHED_DRIVE_FOLDER_ID,
            'link': fmeta.get('webViewLink'),
            'format': 'google_doc',
        })
        write_manifest(m)
        append_log(run_id, f"DRIVE upload ok file_id={m['drive'].get('file_id')}")
    except Exception as e:
        # Do not fail the whole run on Drive issues; log for follow-up.
        append_log(run_id, f"DRIVE upload failed: {e}")
